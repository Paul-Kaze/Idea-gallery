...
Use a hierarchical list format, organized by function levels. Each requirement should include:
- **一级功能（Level 1）:** Top-level feature module
- **二级功能（Level 2）:** Sub-feature under the level 1 module
- **三级功能（Level 3+）:** More detailed sub-features (add more levels as needed)
- **需求具体描述:** Clear, unambiguous description of the requirement
...
|---------|---------|---------|------------|-------|
| 用户管理 | 注册登录 | 邮箱注册 | 用户可通过邮箱+密码完成账号注册 | P0 |
| 用户管理 | 注册登录 | 第三方登录 | 支持 Google/GitHub OAuth 登录 | P1 |
| 用户管理 | 个人设置 | 头像修改 | 用户可上传或选择头像 | P2 |
```

Be explicit and unambiguous. Levels can be extended (四级功能, 五级功能...) based on the complexity of the feature.
...
| 一级功能 | 二级功能 | 三级功能 | 需求具体描述 | 优先级 |
|---------|---------|---------|------------|-------|
| 主题管理 | 配置读取 | - | Theme setting must be readable from `.ralph-tui.yaml` | P0 |
| 主题管理 | 启动应用 | - | Theme must apply on TUI startup | P0 |
| 主题管理 | 实时切换 | - | Theme changes in settings must apply immediately | P0 |
| 主题管理 | 可读性 | - | All text must maintain readability in both themes | P0 |
...
- [ ] Functional requirements are organized by function levels (一级/二级/三级功能) with descriptions and priorities
