...
* 现有迁移文件定义了 `images` 表（supabase/migrations/ai\_gallery.sql:31-64）与 `reference_images` 关系表（supabase/migrations/ai\_gallery.sql:82-95）。本次按需求直接在 `images` 表中维护列表列以简化读路径，保留关系表不动。
