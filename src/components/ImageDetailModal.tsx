  ...
  return (
    ...
    >
      <div className="bg-white max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col md:flex-row relative rounded-[12px]">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-9 h-9 bg-white rounded-full shadow flex items-center justify-center hover:bg-gray-50 transition-colors z-10"
        >
        ...
        {/* Right Side: Information + Operation */}
        <div className="w-full md:w-96 bg-white flex flex-col">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="text-sm text-gray-900">
                  {isPromptExpanded || !shouldShowToggle ? (
                    <p>{item.prompt}</p>
                  ) : (
                    <p>{promptPreview}...</p>
                  )}
                  {shouldShowToggle && (
                    <button
                      ...
                      className="flex items-center gap-1 mt-2 text-sm text-gray-600 hover:text-gray-900 transition-colors"
                    >
                      {isPromptExpanded ? (
                        <>
              ...
              <div>
                <div className="text-xs text-gray-500 mb-2">
                  {item.type === 'video' ? 'Resolution' : 'Dimensions'}
                </div>
                <div className="text-sm text-gray-900">
                  {item.width} × {item.height} px
                  {item.duration && (
                    <span className="ml-2 text-gray-500">({item.duration})</span>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="border-t border-gray-200 p-8">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-6 py-2.5 bg-gray-900 text-white hover:bg-gray-700 transition-colors text-sm"
            >
